...
public class Base64
{
    ...
    public static string Encode(string data, B64MapType mapType, byte offsetChar)
    {
        ...
        switch (mapType)
        {
            case B64MapType.MSPassport:
            {
                Base64Table = Base64PassportTableMap;
                break;
            }
            case B64MapType.MSRegCookie:
            {
                Base64Table = Base64RegCookieTableMap;
                break;
            }
            default:
            {
                Base64Table = Base64DefaultTableMap;
                break;
            }
        }
        ...
        if (data.Length < 4)
        {
            text = new StringBuilder(4);
            padding = 4 - data.Length;
        }
        ...
        if (r != 0)
        {
            text = new StringBuilder(data.Length + (bytes - r));
            padding += r;
        }
        else
        {
            text = new StringBuilder(data.Length);
        }
        ...
        var b64 = new byte[text.Capacity / 3 * 4 + 1];
        ...
        for (var i = 0; i < text.Length; i += 3)
        {
            ...
            b64[c++] += (byte) Base64Table[
                ((text.ToByteArray()[i + 1] & 0x0F) << 2) | (text.ToByteArray()[i + 2] >> 6)];
        ...
        var x = Convert.ToBase64String(b64);
        return Encoding.ASCII.GetString(b64);
    }

    public string Encode()
    {
        return Encode(B64Value, B64MapType.Default, 0);
    }
    ...
    public static string Decode(string data, B64MapType mapType, bool bSkipFirstChar)
    {
        ...
        var Base64ByteMap = new byte[256];

        int padding = 0, c = 0;

        var plainText = new StringBuilder(data);
        var b64 = new byte[data.Length / 4 * 3];

        if (data.Length > 0)
            if (data[data.Length - 1] == Base64Table[64])
            {
                padding++;
                plainText[data.Length - 1] = Base64Table[0];
            }

        if (data.Length > 1)
            if (data[data.Length - 2] == Base64Table[64])
            {
                padding++;
                plainText[data.Length - 2] = Base64Table[0];
            }

        var i = bSkipFirstChar ? 1 : 0;
        ...
        return Encoding.ASCII.GetString(b64, 0, b64.Length - padding);
    }
}