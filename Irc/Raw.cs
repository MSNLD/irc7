...
{
    public static string IRCX_CLOSINGLINK(Server server, User user)
    {
        return $"ERROR :Closing Link: {user}[%s] (%s)";
    }

    public static string IRCX_CLOSINGLINK_007_SYSTEMKILL(Server server, User user)
    {
    ...
    public static string IRCX_CLOSINGLINK_011_PINGTIMEOUT(Server server, User user)
    {
    ...
    public static string RPL_JOIN_IRC(User user, IChannel channel)
    {
        return $":{user.Address} JOIN :{channel}";
    }

    public static string RPL_JOIN_MSN(Server server, User user, IChannel channel)
    {
        return $":{user.Address} JOIN %S :{channel}";
    }

    public static string RPL_PART_IRC(User user, IChannel channel)
    {
    ...
    public static string RPL_MODE_IRC(Server server, User user)
    {
        return $":{user.Address} MODE %s %s";
    }

    public static string RPL_TOPIC_IRC(Server server, User user, IChannel channel)
    {
        return $":{user.Address} TOPIC {channel} :%s";
    }

    public static string RPL_PROP_IRCX(Server server, User user, IChannel channel)
    {
        return $":{user.Address} PROP {channel} %s :%s";
    }

    public static string RPL_KICK_IRC(Server server, User user, IChannel channel)
    {
        return $":{user.Address} KICK {channel} %s :%s";
    }

    public static string RPL_KILL_IRC(Server server, User user)
    {
        return $":{user.Address} KILL %s :%s";
    }
    ...
    public static string RPL_MSG_CHAN(Server server, User user, IChannel channel)
    {
    ...
    public static string RPL_CHAN_WHISPER(Server server, User user, IChannel channel)
    {
        return $":{user.Address} WHISPER {channel} %s :%s";
    }

    public static string RPL_EPRIVMSG_CHAN(Server server, User user, IChannel channel)
    {
        return $":{user.Address} EPRIVMSG {channel} :%s";
    }

    public static string RPL_NOTICE_USER(Server server, User user)
    {
        return $":{user.Address} NOTICE %s :%s";
    }

    public static string RPL_PRIVMSG_USER(Server server, User user)
    {
        return $":{user.Address} PRIVMSG %s :%s";
    }

    public static string RPL_NOTICE_CHAN(Server server, User user, IChannel channel)
    {
        return $":{user.Address} NOTICE {channel} :%s";
    }

    public static string RPL_PRIVMSG_CHAN(User user, IChannel channel, string message)
    {
        return $":{user.Address} PRIVMSG {channel} :{message}";
    }

    public static string RPL_INVITE(Server server, User user, IChannel channel)
    {
        return $":{user.Address} INVITE %s %s :{channel}";
    }

    public static string RPL_KNOCK_CHAN(Server server, User user, IChannel channel)
    {
        return $":{user.Address} KNOCK {channel} %s";
    }

    public static string RPL_NICK(Server server, User user)
    {
        return $":{user.Address} NICK %s";
    }
    ...
    public static string IRCX_RPL_LUSERCLIENT_251(Server server, User user)
    {
        return $":{server} 251 {user} :There are %d users and %d invisible on %d servers";
    }

    public static string IRCX_RPL_LUSEROP_252(Server server, User user)
    {
        return $":{server} 252 {user} %d :operator(s) online";
    }

    public static string IRCX_RPL_LUSERUNKNOWN_253(Server server, User user)
    {
    ...
    public static string IRCX_RPL_LUSERME_255(Server server, User user)
    {
    ...
    public static string IRCX_RPL_LUSERS_265(Server server, User user)
    {
        return $":{server} 265 {user} :Current local users: %d Max: %d";
    }

    public static string IRCX_RPL_GUSERS_266(Server server, User user)
    {
        return $":{server} 266 {user} :Current global users: %d Max: %d";
    }

    public static string IRCX_RPL_AWAY_301(Server server, User user)
    {
    ...
    public static string IRCX_RPL_ENDOFWHO_315(Server server, User user)
    {
    ...
    public static string IRCX_RPL_WHOISIP_320(Server server, User user)
    {
    ...
    public static string IRCX_RPL_TOPIC_332(Server server, User user, IChannel channel, string topic)
    {
    ...
    public static string IRCX_RPL_NAMEREPLY_353(Server server, User user, IChannel channel, string names)
    {
        return $":{server} 353 {user} {channel} :{names}";
    }

    public static string IRCX_RPL_NAMEREPLY_353X(Server server, User user, IChannel channel)
    {
        return $":{server} 353 {user} {channel} :";
    }

    public static string IRCX_RPL_ENDOFNAMES_366(Server server, User user, IChannel channel)
    {
        return $":{server} 366 {user} %s :End of /NAMES list.";
    }

    public static string IRCX_RPL_BANLIST_367(Server server, User user, IChannel channel)
    {
        return $":{server} 367 {user} {channel} %s";
    }

    public static string IRCX_RPL_ENDOFBANLIST_368(Server server, User user, IChannel channel)
    {
    ...
    public static string IRCX_ERR_NOSUCHNICK_401(Server server, User user)
    {
    ...
    public static string IRCX_ERR_CANNOTSENDTOCHAN_404(Server server, User user)
    {
        return $":{server} 404 {user} %s :Cannot send to channel";
    }

    public static string IRCX_ERR_CANNOTSENDTOCHAN_404X(Server server, User user, IChannel channel)
    {
        return $":{server} 404 {user} {channel} :Cannot send to channel";
    }

    public static string IRCX_ERR_TOOMANYCHANNELS_405(Server server, User user, IChannel channel)
    {
    ...
    public static string IRC_ERR_NORECIPIENT_411(Server server, User user)
    {
        return $":{server} 411 {user} :No recipient given (%s)";
    }

    public static string IRC_ERR_NOTEXT_412(Server server, User user)
    {
    ...
    public static string IRCX_ERR_ERRONEOUSNICK_432(Server server, User user)
    {
    ...
    public static string IRCX_ERR_NONICKCHANGES_439(Server server, User user)
    {
        return $":{server} 439 {user} %s :Nick name changes not permitted.";
    }

    public static string IRCX_ERR_NOTONCHANNEL_442(Server server, User user, IChannel channel)
    {
        return $":{server} 442 {user} {channel} :You're not on that channel";
    }

    public static string IRCX_ERR_USERONCHANNEL_443(Server server, User user, IChannel channel)
    {
    ...
    public static string IRCX_ERR_KEYSET_467(Server server, User user, IChannel channel)
    {
        return $":{server} 467 {user} {channel} :Channel key already set";
    }

    public static string IRCX_ERR_CHANNELISFULL_471(Server server, User user, IChannel channel)
    {
        return $":{server} 471 {user} {channel} :Cannot join channel (+l)";
    }

    public static string IRCX_ERR_UNKNOWNMODE_472(Server server, User user)
    {
        return $":{server} 472 {user} %l :is unknown mode char to me";
    }

    public static string IRCX_ERR_INVITEONLYCHAN_473(Server server, User user, IChannel channel)
    {
        return $":{server} 473 {user} {channel} :Cannot join channel (+i)";
    }

    public static string IRCX_ERR_BANNEDFROMCHAN_474(Server server, User user, IChannel channel)
    {
        return $":{server} 474 {user} {channel} :Cannot join channel (+b)";
    }

    public static string IRCX_ERR_BADCHANNELKEY_475(Server server, User user, IChannel channel)
    {
        return $":{server} 475 {user} {channel} :Cannot join channel (+k)";
    }

    public static string IRCX_ERR_NOPRIVILEGES_481(Server server, User user, IChannel channel)
    {
        return $":{server} 481 {user} :Permission Denied - You're not an IRC operator";
    }

    public static string IRCX_ERR_CHANOPRIVSNEEDED_482(Server server, User user, IChannel channel)
    {
        return $":{server} 482 {user} {channel} :You're not channel operator";
    }

    public static string IRCX_ERR_CHANQPRIVSNEEDED_485(Server server, User user, IChannel channel)
    {
    ...
    public static string IRCX_ERR_AUTHONLYCHAN_556(Server server, User user, IChannel channel)
    {
        return $":{server} 556 {user} {channel} :Only authenticated users may join channel.";
    }

    public static string IRCX_ERR_SECUREONLYCHAN_557(Server server, User user, IChannel channel)
    {
    ...
    public static string IRCX_RPL_LISTRLIST_614(Server server, User user, IChannel channel)
    {
    ...
    public static string IRC2_RPL_WHOISSECURE_671(Server server, User user)
    {
    ...
    public static string IRCX_RPL_ACCESSADD_801(Server server, User user)
    {
        return $":{server} 801 {user} %s %s %s %d %s :%s";
    }

    public static string IRCX_RPL_ACCESSDELETE_802(Server server, User user)
    {
        return $":{server} 802 {user} %s %s %s %d %s :%s";
    }

    public static string IRCX_RPL_ACCESSSTART_803(Server server, User user)
    {
        return $":{server} 803 {user} %s :Start of access entries";
    }

    public static string IRCX_RPL_ACCESSLIST_804(Server server, User user)
    {
        return $":{server} 804 {user} %s %s %s %d %s :%s";
    }

    public static string IRCX_RPL_ACCESSEND_805(Server server, User user)
    {
    ...
    public static string IRCX_RPL_LISTXLIST_812(Server server, User user)
    {
    ...
    public static string IRCX_RPL_PROPLIST_818(Server server, User user)
    {
        return $":{server} 818 {user} %s %s :%s";
    }

    public static string IRCX_RPL_PROPEND_819(Server server, User user)
    {
        return $":{server} 819 {user} %s :End of properties";
    }

    public static string IRCX_RPL_ACCESSCLEAR_820(Server server, User user)
    {
    ...
    public static string IRCX_RPL_USERNOWAWAY_822(Server server, User user)
    {
        return $":{user.Address} 822 :%s";
    }
    ...
    public static string IRCX_ERR_BADCOMMAND_900(Server server, User user)
    {
        return $":{server} 900 {user} %s :Bad command";
    }

    public static string IRCX_ERR_TOOMANYARGUMENTS_901(Server server, User user)
    {
    ...
    public static string IRCX_ERR_BADLEVEL_903(Server server, User user)
    {
        return $":{server} 903 {user} %s :Bad level";
    }

    public static string IRCX_ERR_BADPROPERTY_905(Server server, User user)
    {
    ...
    public static string IRCX_ERR_NOACCESS_913(Server server, User user)
    {
    ...
    public static string IRCX_ERR_NOWHISPER_923(Server server, User user, IChannel channel)
    {
        return $":{server} 923 {user} {channel} :Does not permit whispers";
    }

    public static string IRCX_ERR_NOSUCHOBJECT_924(Server server, User user)
    {
        return $":{server} 924 {user} %s :No such object found";
    }

    public static string IRCX_ERR_ALREADYONCHANNEL_927(Server server, User user, IChannel channel)
    {
    ...
    public static string IRCX_ERR_TOOMANYINVITES_929(Server server, User user)
    {
