...
namespace Irc.Extensions.Apollo.Tests
{
    internal class GateKeeperPassportTests
    {
        ...
        public void AcceptSecurityContext_V3_Auth_Succeeds_With_Cookies()
        {
            ...
            Assert.AreEqual(EnumSupportPackageSequence.SSP_CREDENTIALS, gateKeeperPassport.AcceptSecurityContext(token, ip));

            gateKeeperToken.Version = 3;
            gateKeeperToken.Sequence = (int)EnumSupportPackageSequence.SSP_CREDENTIALS;

            // Token with ticket & profile cookies
            token = $"{gateKeeperToken.Serialize<GateKeeperToken>().ToAsciiString()}{passportCredentials.Ticket.Length:X8}{passportCredentials.Ticket}{passportCredentials.Profile.Length:X8}{passportCredentials.Profile}";
