   ...
   public class FloodProtectionLevel
    {
        ...
        public long LastProcessedTicks, LastProcessedData, LastProcessedInvite, LastProcessedJoin, LastProcessedPassword, LastProcessedStandard, LastProcessedHostMessage;
        ...
        {
            long offset = 0x089f7ff5f7b58000; //01/01/1970
        ...
        public bool CanProcess
        {
            get
            {
                if (Delay > 0)
                {
                    if (((DateTime.UtcNow.Ticks - LastProcessedTicks) / TimeSpan.TicksPerSecond) >= Delay) { LastProcessedTicks = DateTime.UtcNow.Ticks; return true; }
                    else { return false; }
                }
                else { return true; }
            }
        ...
        public bool CanProcessData
        {
            get
            {
                if (DelayData > 0)
                {
                    if (((DateTime.UtcNow.Ticks - LastProcessedData) / TimeSpan.TicksPerSecond) >= DelayData) { LastProcessedData = DateTime.UtcNow.Ticks; return true; }
                    else { return false; }
                }
                else { return true; }
            }
        ...
        public bool CanProcessInvite
        {
            get
            {
                if (((DateTime.UtcNow.Ticks - LastProcessedInvite) / TimeSpan.TicksPerSecond) >= DelayInvite) { LastProcessedInvite = DateTime.UtcNow.Ticks; return true; }
                else { return false; }
            }
        }
        public bool CanProcessJoin
        {
            get
            {
                if (((DateTime.UtcNow.Ticks - LastProcessedJoin) / TimeSpan.TicksPerSecond) >= DelayJoin) { LastProcessedJoin = DateTime.UtcNow.Ticks; return true; }
                else { return false; }
            }
        }
        public bool CanProcessPassword
        {
            get
            {
                if (((DateTime.UtcNow.Ticks - LastProcessedPassword) / TimeSpan.TicksPerSecond) >= DelayWrongPass) { LastProcessedPassword = DateTime.UtcNow.Ticks; return true; }
                else { return false; }
            }
        }
        public bool CanProcessStandard
        {
            get
            {
                if (((DateTime.UtcNow.Ticks - LastProcessedStandard) / TimeSpan.TicksPerSecond) >= DelayStandard) { LastProcessedStandard = DateTime.UtcNow.Ticks; return true; }
                else { return false; }
            }
        }
        public bool CanProcessHostMessage
        {
            get
            {
                ...
                {
                    if (((DateTime.UtcNow.Ticks - LastProcessedHostMessage) / TimeSpan.TicksPerSecond) >= DelayHostMessage) { LastProcessedHostMessage = DateTime.UtcNow.Ticks; return true; }
                    else { return false; }
                }
                else { return true; }
            }
    ...
    public enum ProtectionLevel { ctProtectionNone, ctProtectionLow, ctProtectionMedium, ctProtectionHigh };
    ...
    public class FloodProfile
    {
        ...
        public SAT_RESULT Saturation
        {
            get
            {
                if (currentInputBytes > InputFloodLimit) { return SAT_RESULT.S_INPUT_EXCEEDED; }
                else if (currentOutputBytes > OutputSaturationLimit) { return SAT_RESULT.S_OUTPUT_EXCEEDED; }
                else { return SAT_RESULT.S_OK; }
            }
        }
    }

    public enum SAT_RESULT { S_OK, S_OUTPUT_EXCEEDED, S_INPUT_EXCEEDED };
    public enum FLD_RESULT { S_OK, S_WAIT };
    public class Flood
    {
        ...
        public static FLD_RESULT Audit(FloodProfile profile, CommandDataType type, UserAccessLevel level)
        {
            ...
            switch (type)
            {
                case CommandDataType.None:
                    {
                        if (profile.FloodProtectionLevel.CanProcess) { return FLD_RESULT.S_OK; }
                        else { return FLD_RESULT.S_WAIT; }
                    }
                case CommandDataType.Data:
                    {
                        if (profile.FloodProtectionLevel.CanProcessData) { return FLD_RESULT.S_OK; }
                        else { return FLD_RESULT.S_WAIT; }
                    }
                case CommandDataType.Invitation:
                    {
                        if (profile.FloodProtectionLevel.CanProcessInvite) { return FLD_RESULT.S_OK; }
                        else { return FLD_RESULT.S_WAIT; }
                    }
                case CommandDataType.Join:
                    {
                        if (profile.FloodProtectionLevel.CanProcessJoin) { return FLD_RESULT.S_OK; }
                        else { return FLD_RESULT.S_WAIT; }
                    }
                case CommandDataType.WrongChannelPassword:
                    {
                        if (profile.FloodProtectionLevel.CanProcessPassword) { return FLD_RESULT.S_OK; }
                        else { return FLD_RESULT.S_WAIT; }
                    }
                case CommandDataType.Standard:
                    {
                        if (profile.FloodProtectionLevel.CanProcessStandard) { return FLD_RESULT.S_OK; }
                        else { return FLD_RESULT.S_WAIT; }
                    }
                case CommandDataType.HostMessage:
                    {
                        if (profile.FloodProtectionLevel.CanProcessHostMessage) { return FLD_RESULT.S_OK; }
                        else { return FLD_RESULT.S_WAIT; }
                    }
                default: { return FLD_RESULT.S_OK; }
            }
        }
    }
}
