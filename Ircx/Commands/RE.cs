        ...
        public new COM_RESULT Execute(Frame Frame)
        {
            ...
            else
            {
                for (int i = 0; i < Frame.Server.Users.Length; i++)
                {
                    ...
                    if (User.Registered)
                    {
                        if (String8RegEx.EvaluateString8(Frame.Message.Data[0], User.Address.Nickname, true))
                        {
                            if (User.ChannelList.Count == 0)
                            {
                                Frame.User.Send(Raws.Create(Server: Frame.Server, Client: Frame.User, Raw: Raws.IRCX_RPL_REVEAL_851, Data: new String8[] { User.Address.Nickname, User.OIDX8, User.Address.RemoteIP, User.Address._address[1], Resources.Null }));
                            }
                            else
                            {
                                for (int x = 0; x < User.ChannelList.Count; x++)
                                {
                                    Frame.User.Send(Raws.Create(Server: Frame.Server, Client: Frame.User, Raw: Raws.IRCX_RPL_REVEAL_851, Data: new String8[] { User.Address.Nickname, User.OIDX8, User.Address.RemoteIP, User.Address._address[1], User.ChannelList[x].Channel.Name }));
                                }
                            }
                        }
                    }
