...
internal class ACCESS : Command
{
        ...
        if (message.Parameters.Count >= 1)
        {
            ...
            if (obj != null)
                switch (obj)
                {
                    case Channel:
                    {
                        var objType = IrcHelper.IdentifyObject(AccessObjectName);
                ...
                if (UserLevel >= requiredLevel)
                {
                    ...
                    else if (message.Parameters.Count >= 2)
                    {
                        ...
                        if (Operator != EnumAccessOperator.NONE)
                        {
                            ...
                            {
                                ProcessClear(server, AccessObject as Access, user, UserLevel, message, Level);
                            }
                            else if (Operator == EnumAccessOperator.ADD)
                            {
                                if (Level != AccessLevel.None)
                                    //attempt to process request
                                    ProcessAdd(server, AccessObject as Access, user, UserLevel, message, Level);
                                else
                            ...
                            {
                                if (Level != AccessLevel.None)
                                    ProcessDelete(server, AccessObject as Access, user, UserLevel, message, Level);
                                //attempt to process request
                                else
                                    //<- :Default-Chat-Community 461 Sky ACCESS :Not enough parameters
                                    user.Send(RawBuilder.Create(server, Client: user, Raw: Raws.IRCX_ERR_NEEDMOREPARAMS_461,
    ...
    public void ProcessList(Server server, Access Access, User user, Message message)
    {
        if (message.Parameters.Count <= 3)
        {
             ...
             */
            user.Send(RawBuilder.Create(server, Client: user, Raw: Raws.IRCX_RPL_ACCESSSTART_803,
                Data: new[] {Access.ObjectName}));

            for (var i = 0; i < Access.Entries.Entries.Count; i++)
                user.Send(RawBuilder.Create(server, Client: user, Raw: Raws.IRCX_RPL_ACCESSLIST_804, Data: new[]
                    {
                        Access.ObjectName,
            ...
            user.Send(RawBuilder.Create(server, Client: user, Raw: Raws.IRCX_RPL_ACCESSEND_805,
                Data: new[] {Access.ObjectName}));
        }
        else
        {
        ...
        AccessLevel level)
    {
        ...
        if (!bException)
            ...
            user.Send(RawBuilder.Create(server, Client: user, Raw: Raws.IRCX_RPL_ACCESSCLEAR_820,
                Data: new[]
                {
            ...
            user.Send(RawBuilder.Create(server, Client: user, Raw: Raws.IRCX_ERR_ACCESSNOTCLEAR_922));
    }
        ...
        if (message.Parameters.Count >= 4)
        {
            ...
            if (Mask.Parse(message.Parameters[3]))
            {
                if (Access.Entries.Contains(Mask) == null)
                {
                    ...
                    {
                        user.Send(RawBuilder.Create(server, Client: user, Raw: Raws.IRCX_ERR_TOOMANYACCESSES_916));
                ...
                {
                    //<- :Default-Chat-Community 914 Sky :Duplicate access entry
                    user.Send(RawBuilder.Create(server, Client: user, Raw: Raws.IRCX_ERR_DUPACCESS_914));
                }
            }
            else
            {
        ...
        if (message.Parameters.Count >= 3)
        {
            ...
            if (Mask.Parse(message.Parameters[3]))
            {
                ...
                else
                {
                    ...
                    user.Send(RawBuilder.Create(server, Client: user, Raw: Raws.IRCX_ERR_MISACCESS_915));
                }
            }
            else
            {
