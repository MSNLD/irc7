...
namespace Irc.Extensions.NTLM
{
    ...
    {
        string NTLM_DES_CONST = new string("KGS!@#$%");
        ...
        private byte[] NtowFv2(string unicodePassword, string unicodeUsername, string unicodeDomain)
        {
            ...
            HMACMD5 hmacmd5 = new HMACMD5(md4Password);
            return hmacmd5.ComputeHash((unicodeUsername + unicodeDomain).ToByteArray());
        }

        public string NtlmV2Response(string unicodeUsername, string unicodePassword,  string serverChallenge, string dataBlob)
        {
            ...
            if (string.IsNullOrWhiteSpace(dataBlob)) throw new ArgumentException("Data Blob (NTLM Parameter) cannot be empty");

            NTLMShared.NTLMv2Blob ntlMv2Blob = new NTLMShared.NTLMv2Blob(dataBlob);
            ...
            HMACMD5 hmacmd5 = new HMACMD5(ResponseKeyNT);
        ...
        public string LmResponse(string password, string challenge)
        {
            ...
            if (string.IsNullOrWhiteSpace(challenge)) throw new ArgumentException("Challenge cannot be empty");
            if (password.Length > LmMaxPasswordLength) throw new ArgumentException($"Password cannot more than {LmMaxPasswordLength} characters");
            ...
            string hash1 = desEncryptor.Encrypt(NTLM_DES_CONST, key1Bytes); 
            string hash2 = desEncryptor.Encrypt(NTLM_DES_CONST, key2Bytes);
