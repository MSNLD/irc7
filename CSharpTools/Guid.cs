...
namespace CSharpTools
{
    public class GUID
    {
        ...
        public GUID(byte[] b) { Initialize(); SetBytes(b); }
        ...
        public void SetBytes(byte[] b)
        {
            ...
            for (int c = 0; c < 8; c++) { data4[c] = b[8 + c]; }
        }
        ...
        public byte[] ToHex()
        {
            ...
            for (int c = 0; c < 8; c++) { b[8 + c] = data4[c]; }

            byte[] _iGuid = new byte[b.Length * 2];
    ...
    public class GuidNode
    {
        ...
        public GuidNode() { }
        public GuidNode(GUID guida) { guid = guida; count++; }
    }
    public class GuidMap
    {
        List<GuidNode> guid;

        public GuidMap() { guid = new List<GuidNode>(); }

        public GuidNode FindGuid(GUID guida)
        {
            for (int c = 0; c < guid.Count; c++) { if (guid[c].guid == guida) { return guid[c]; } }
            return null;
        }
        public GuidNode AddGuid(GUID guida)
        {
            GuidNode node = FindGuid(guida);
            if (node == null) { node = new GuidNode(guida); }
            else { node.count++; }
            guid.Add(node);
